...
isCase :: Comm -> Bool
isCase (IfThenElse b c1 Skip) = True
isCase (IfThenElse b c1 rest) = isCase rest
isCase _ = False
...
pCase (Case xs) = 
  text "case" 
    <+> lbrace 
    $$ nest tabW (foldr ($$) empty (map format xs))
    $$ rbrace where
  format (b, c) = pExp b <+> text ":" <+> braces (pComm c)
